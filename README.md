[![DOI](https://zenodo.org/badge/1309152761.svg)](https://doi.org/10.5281/zenodo.22102932)
# **A computational model of human echolocation using a Kalman filter**

Krasovskaya, S., Coughlan, M. J., Teng, S., 2026

A 2D azimuth-based simulation of blind human echolocation using a Kalman filter. Simulates test and control conditions from \[Patel VSS 2024; Teng & Fusco, 2026], aiming to model how information is accumulated across echolocation clicks to estimate a target location. The computational model integrates Bayesian belief estimation using a Kalman filter (KF), binaural audio processing, and motor control to simulate how people localize targets using self-generated click sounds. The KF continuously updates a belief estimate of target location from echo-derived ITD measurements, with movement bias scaling proportionally to KF confidence.

Three experimental conditions are simulated:
1. Control — no acoustic feedback; head explores freely
2. Big target — audio feedback, high-reflectivity target (29 × 36 cm)
3. Small target — audio feedback, low-reflectivity target (2.5 × 17 cm)

## Structure:
```
**---Parameter generation---**
└── 1-generate_subs.py                          # Generate experimental_setup.json containing subject parameters and target positions 
 
**---Model---**
├── KF.py                                       # KF functions (kf_predict, kf_update)
├── model_core.py                               # Core classes and shared utility functions
├── model_engine.py                             # Simulation and experiment runner functions
└── model_launch.py                             # Entry point. Edit this file and run.

**---Analysis---**
└── analysis/    
    ├── 3.1-success_rates_&_RTs.py              # Success rates and response time analysis
	├── 3.2-trajectories.py                     # Head orientation trajectory visualizations
	├── 3.3-convergence.py                      # Error convergence analysis
    ├── 3.4-plot_final_head_az_distro.py        # Plot distribution of final head azimuths: Big vs Small target
	├── 3.5-plot_theta_vs_theta_target.py   	# Actual vs. desired head position (θ vs θ\_target)
	└── 3.6-plot_P_over_time.py                 # KF uncertainty (P) over time: Big vs. Small

**---Outputs---**
*Simulation Outputs*
├── experimental_setup.json                     # Generated by 1-generate\_subs.py (required before running conditions)
├── sim_control_condition/                      # Output folder: control results
├── sim_test_big/                               # Output folder: big target results
└── sim_test_small/                             # Output folder: small target results

*Analysis Outputs*
└── analysis/
    └──analysis_results/
        ├── convergence_analysis_start_lock.png
        ├── convergence_comparisons_stats.csv
        ├── convergence_summary_stats.csv
        ├── final_head_vs_target.png
        ├── final_error_distribution.png
        ├── final_error_stats.csv
        ├── objective_success_rates.png
        ├── response_times.png
        ├── rt_stats.csv
        ├── success_rates_detailed.csv
        ├── success_rates_stats.csv
        ├── trajectory_comparison_50.png
        └── trajectory_comparison_ALL_end_lock.png
```
_____________________________________________________________________________

## How to run the simulation:
### Step 0 — generate subjects (once only)
Run 1-generate_subs.py to create experimental_setup.json. This file defines
subject parameters, target positions, and the random seed used across all three
conditions to ensure consistency.

### Step 1 — configure
Open model_launch.py. Edit the three sections at the top:
```
 1. Set condition
CONDITION = 'big'           # 'control', 'big', or 'small'

 2. Set path to audio files
AUDIO_DIR = "C:/path/to/wavs_1deg"

 3. Choose what to run
RUN_AUDIO_TEST   = False   # verify audio files load correctly
RUN_SINGLE_TRIAL = False   # diagnostic single-trial run
RUN_EXPERIMENT   = True    # full multi-trial experiment
```
### Step 2 — Run
Then press Run. Output is saved automatically to the appropriate folder
(`sim_control_condition`, `sim_test_big`, or `sim_test_small`).

_____________________________________________________________________________

## Audio file naming
The simulation expects stereo .wav files named by angle, e.g.:
`0deg.wav`, `-45.wav`, `angle_15.wav`, `az-30_tdist100_Idist100_fs96000.wav`
_____________________________________________________________________________

## Analysis scripts
Run these after all three conditions have been simulated. Each script loads the
JSON results files and produces figures and tables with results.
